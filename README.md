...
| Entity cross-linking | "Zhengzhou" → needs to find "Mother-in-law Project" via graph traversal |
...
A persistent entity graph (Kuzu embedded graph DB) that enables multi-hop retrieval across entities. Solves the problem where "Zhengzhou" and "Mother-in-law Project" live in different memories with no shared keyword.
