...
DEEPSEEK_MODEL   = "deepseek:deepseek-v4-pro" # DeepSeek 官方雲端（.env 的 LLM_API_KEY）
...
def pick_backend(forced: str) -> tuple[str, str, object | None]:
    ...
    if forced == "local":
        ...
        print("│  [4] 雲端 DeepSeek（deepseek-v4-pro）    │")
        print("└─────────────────────────────────────────┘")
        try:
    ...
    if choice == "2":
        ...
        return "deepseek", DEEPSEEK_MODEL, None
    else:
...
def main():
    ...
    parser.add_argument("--backend",    type=str, default="",      help="local / cloud / proxy / deepseek（省略則互動選擇）")
    ...
    if backend == "local":
        ...
        backend_label = f"DeepSeek / {model}"
    else:
